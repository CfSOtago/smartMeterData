...
In theory this should be half-hourly electricity consumtion (Wh or kWh) data for a large(ish) number of TOngan households. It even be all Tongan households with smart meters. But who knows...
...
```{r plot_f1_NumberOfX1IDsByDate, fig.cap="Number of unique X.id.1 by date"}
plotDT <- f1_dt[, .(nX.id.1 = uniqueN(X.id.1)), keyby = .(date)]
ggplot2::ggplot(plotDT, aes(y = nX.id.1, x = date)) +
  geom_point()
```

```{r plot_f1_NumberOfObsByDateTime, fig.cap="Number of obs by date"}
plotDT <- f1_dt[, .(nObs = .N), keyby = .(date)]
ggplot2::ggplot(plotDT, aes(y = nObs, x = date)) +
  geom_point()
```

Yeah. So this does suggest that X.id.2 is a household ID as it follows the pattern of the number of observations (\@ref(fig:plot_f1_NumberOfObsByDateTime)).
...
```{r plot_f1_value, fig.cap="Distribution of Value"}
skim(f1_dt$Value)
...
Well \@ref(fig:plot_f1_value) looks a bit odd. Why is there a sort of threshold at 250? We obviously have very few Values over 250...
...
```{r plot_f1_meanValueByTime, fig.cap = "Mean Value by time of day"}
...
What is this? Is this even the right shape?? \@ref(fig:plot_f1_meanValueByTime) Could be... but the values are weird.
